...
q_btn.addEventListener("click",e =>{
	if(category.value === "query" && cat_query.value === "bible"){
		...
		document.getElementById("question").innerHTML = `${Bible_Q_A[qq].Question}  Ans: ${Bible_Q_A[qq].Answer}`;
	}else if(category.value === "query" && cat_query.value === "trivia"){
		qq = Number(slider.value) - 1;
		document.getElementById("question").innerHTML = `${Fun_General[qq].Question} Ans : ${Fun_General[qq].Answer}`;
